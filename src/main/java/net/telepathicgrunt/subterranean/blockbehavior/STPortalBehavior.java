package net.telepathicgrunt.subterranean.blockbehavior;
		...
		public static void BlockRightClickEvent(PlayerInteractEvent.RightClickBlock event)
		{
			...
			if (event.getWorld().getBlockState(event.getPos()) == STBlocks.THIN_AIR.get().getDefaultState())
			{
				...
				if (!world.isRemote && !entityIn.isPassenger() && !entityIn.isBeingRidden() && entityIn.isNonBoss() && !((PlayerEntity) entityIn).isCrouching())
				{
					...
					((ServerPlayerEntity) entityIn).teleport(minecraftserver.getWorld(destination), blockpos.getX() + 0.5D, blockpos.getY() + 1, blockpos.getZ() + 0.5D, entityIn.rotationYaw, entityIn.rotationPitch);
				}
			}
